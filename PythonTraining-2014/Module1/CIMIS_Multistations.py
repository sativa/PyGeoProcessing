...
import numpy, os, csv,zipfile, urllib
from itertools import product



# data reading , cleaning and adding header 
year = input("Please Input Year for ETr and ETo calculations ")
datapath='E:/Teaching/Python/GIS-WR/Module1/Data'
os.chdir(datapath)

stations = numpy.genfromtxt('stations.csv', delimiter=',',names=True)

u1='ftp://ftpcimis.water.ca.gov/pub/annualMetric/dailyStns'
url=u1+str(year)+'.zip'
         
print "Downloading Data for "+ str(year) 
fname='dailyStns'+str(year)+'.zip'
#urllib.urlretrieve(url, str(fname))
zipdata = zipfile.ZipFile(fname)
zipdata.extractall(r'weatherdata'+str(year))
os.chdir("weatherdata"+str(year))     
...
for filename in os.listdir(os.getcwd()):
            ...
            if extension.lower() == ".csv":
                ...
                with open(filename,"rb") as source:
                    ...
                    newfile='Output/new_'+str(filename)
                    ...
                    with open(newfile,"wb") as result:
                        ...
                        for r in rdr:
                            wtr.writerow( (r[0], r[1], r[2], r[4], r[6], r[8], r[10], r[12], r[14], r[16], r[18], r[20], r[22], r[24], r[26], r[28], r[30]) )
print ("We are done with data pre-processing")

os.chdir('Output')
if not os.path.exists("ET"):
    os.makedirs("ET")
i=0
for filename, i in product(os.listdir(os.getcwd()),range(1,11)) :

    if 'new_' in filename:
            my_data = numpy.genfromtxt(filename, delimiter=',',names=True)
            #Read latitude longitude,elevation  variables automatically from station file 
            Station_id=stations['Stn'][i]
   
            latitude=stations['Lat'][i]
            longitude=stations['Long'][i]
            elev=stations['ELEV'][i]
            print (Station_id, elev)
            ...
            fname='ET/dailyETr_'+str(year)+str(Station_id)+'.csv'
            myoutputfile=open(fname, "wb" )
